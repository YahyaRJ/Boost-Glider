...
Design_Input = readtable("Design Input File9.xlsx",'Sheet','Input','ReadRowNames',true); %Read in Aircraft Geometry File

Count = height(Design_Input); %Number of different aircraft configurations in geometry file

% Import Airfoil Data File
Airfoil = readtable("Design Input File9.xlsx",'Sheet','Airfoil_Data'); %Read in Airfoil Data

% Import Benchmark Aircraft Truth Data
Benchmark = readtable("Design Input File9.xlsx",'Sheet','Benchmark_Truth'); %Read in Benchmark "Truth" Data for model validation only

% Import Material Properties Data
Material_Data = readtable("Design Input File9.xlsx",'Sheet','Materials'); %Read in prototyp material densities for weight model
...
figure();
yyaxis left;
plot(apogee(2:101),"LineWidth",2);
ylabel('Rocket Apogee(m)');
yyaxis right;
plot(GlideRange{2:101, 1},"LineWidth",2);
ylabel('Glide Gange(m)');
xlabel('Horizontal Area');
title('Horizontal Stabilizer Optimization');
